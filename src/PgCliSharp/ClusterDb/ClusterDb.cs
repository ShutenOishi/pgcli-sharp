...
/// <summary><para>EN: Typed clusterdb options for PostgreSQL 10-18.</para><para>JA: PostgreSQL 10〜18 の型付き clusterdb オプションです。</para></summary>
public sealed class ClusterDbOptions
{
    ...
    /// <summary><para>EN: Gets or sets verbose CLUSTER.</para><para>JA: verbose CLUSTER を取得または設定します。</para></summary>
...
public sealed class ClusterDb
{
    ...
    public ClusterDb(string executablePath, PostgreSqlMajorVersion version) : this(executablePath, version, new ProcessRunner()) { }
    internal ClusterDb(string executablePath, PostgreSqlMajorVersion version, IProcessRunner runner) => _executor = new MaintenanceExecutor(executablePath, version, runner);
...
#endif
        ClusterdbValidator.Validate(options, Version);
        IReadOnlyList<string> arguments = ClusterdbArgumentBuilder.Build(options);
