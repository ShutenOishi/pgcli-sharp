...
/// <summary>
/// <para>EN: Identifies how a PostgreSQL 17+ pg_restore filter is supplied.</para>
/// <para>JA: PostgreSQL 17 以降の pg_restore フィルターの供給方法を表します。</para>
/// </summary>
public enum PgRestoreFilterSourceKind
{
...
/// <summary>
/// <para>EN: Represents a PostgreSQL 17+ pg_restore filter source.</para>
/// <para>JA: PostgreSQL 17 以降の pg_restore フィルター入力元を表します。</para>
/// </summary>
public sealed class PgRestoreFilterSource
{
    private PgRestoreFilterSource(PgRestoreFilterSourceKind kind, string? path, Stream? input)
    {
        Kind = kind;
        Path = path;
        Input = input;
    ...
    public PgRestoreFilterSourceKind Kind { get; }

    /// <summary><para>EN: Gets the filter file path for file input.</para><para>JA: ファイル入力の場合のフィルターファイルパスを取得します。</para></summary>
    ...
    public static PgRestoreFilterSource FromFile(string path)
    {
        ...
        return new PgRestoreFilterSource(PgRestoreFilterSourceKind.File, path, null);
    }
    ...
    /// <returns><para>EN: The filter source.</para><para>JA: フィルター入力元です。</para></returns>
    public static PgRestoreFilterSource FromStandardInput(Stream input)
    {
#if NETSTANDARD2_0
        if (input is null)
        {
        ...
        return new PgRestoreFilterSource(PgRestoreFilterSourceKind.StandardInput, null, input);
    }
}
